...
import org.mifosplatform.portfolio.loanaccount.exception.DsaOfficerAssignmentException;
...
public class Loan extends AbstractPersistable<Long> {
    ...
    @ManyToOne
    @JoinColumn(name = "dsa_officer_id", nullable = true)
    private Dsa dsaOfficer;
    ...
    @Column(name = "advanceEmi_N", scale = 6, precision = 19 , nullable = true )
    ...
    public static Loan newIndividualLoanApplication(final String accountNo, final Client client, final Integer loanType,
            final LoanProduct loanProduct, final Fund fund, final Staff officer,final Dsa dsaOfficer, final CodeValue loanPurpose,
            ...
            final BigDecimal maxOutstandingLoanBalance, final Boolean createStandingInstructionAtDisbursement,final Boolean isFloatingInterestRate, final BigDecimal interestRateDifferential,final Integer advanceEmiN) {

        final LoanStatus status = null;
        final Group group = null;
        final Boolean syncDisbursementWithMeeting = null;
        return new Loan(accountNo, client, group, loanType, fund, officer,dsaOfficer, loanPurpose, transactionProcessingStrategy, loanProduct,
                loanRepaymentScheduleDetail, status, loanCharges, collateral, syncDisbursementWithMeeting, fixedEmiAmount,
                disbursementDetails, maxOutstandingLoanBalance, createStandingInstructionAtDisbursement,isFloatingInterestRate,interestRateDifferential,advanceEmiN);
    }
    ...
    public static Loan newIndividualLoanApplicationFromGroup(final String accountNo, final Client client, final Group group,
            final Integer loanType, final LoanProduct loanProduct, final Fund fund, final Staff officer, final Dsa dsaOfficer,final CodeValue loanPurpose,
            final LoanTransactionProcessingStrategy transactionProcessingStrategy,
            final LoanProductRelatedDetail loanRepaymentScheduleDetail, final Set<LoanCharge> loanCharges,
            final Set<LoanCollateral> collateral, final Boolean syncDisbursementWithMeeting, final BigDecimal fixedEmiAmount,
            final Set<LoanDisbursementDetails> disbursementDetails, final BigDecimal maxOutstandingLoanBalance,
    ...
    private Loan(final String accountNo, final Client client, final Group group, final Integer loanType, final Fund fund,
            final Staff loanOfficer, final Dsa dsaOfficer, final CodeValue loanPurpose, final LoanTransactionProcessingStrategy transactionProcessingStrategy,
        ...
        this.netDisbursedAmount = BigDecimal.ZERO;
    ...
    public BigDecimal getTotalPrincipalPaid(){
    	return this.loanSummaryWrapper.calculateTotalPrincipalRepaid(this.repaymentScheduleInstallments, getCurrency()).getAmount();
    }
    ...
    private Money calculateInstallmentChargeAmount(final ChargeCalculationType calculationType, final BigDecimal percentage,
        ...
        switch (calculationType) {
            ...
            case PERCENT_OF_OUTSTANDING_PRINCIPAL:
            	percentOf = installment.getTotalPrincipalOutstanding(getCurrency());
            break;
            default:
            break;
        }
    ...
    public void updateDsaOfficerOnLoanApplication(final Dsa newDsaOfficer){
        if (!isSubmittedAndPendingApproval()){
            Long dsaOfficerId = null;
            if(this.dsaOfficer != null){
                dsaOfficerId = this.dsaOfficer.getId();
            }
                throw new DsaOfficerAssignmentException(getId(),dsaOfficerId);
        }

        this.dsaOfficer = newDsaOfficer;
    }
    ...
    private Money calculateOverdueAmountPercentageAppliedTo(LoanRepaymentScheduleInstallment installment,
        ...
        switch (calculationType) {
            ...
            break;
            case PERCENT_OF_OUTSTANDING_PRINCIPAL:
            	amount = installment.getTotalPrincipalOutstanding(getCurrency());
            break;
            default:
            break;
        }
    ...
    public Map<String, Object> loanApplicationApproval(final AppUser currentUser, final JsonCommand command,
        ...
        if (!newStatusEnum.hasStateOf(LoanStatus.fromInt(this.loanStatus))) {
            ...
            if(this.advanceEmiN > 0 ){
            	...
            	BigDecimal findingEmi = (findingInterestRate.add(this.approvedPrincipal)).divide(new BigDecimal(loanRepaymentScheduleDetail.getNumberOfRepayments()),0,RoundingMode.HALF_UP);   
            	
            	this.fixedEmiAmount = findingEmi;
            	BigDecimal emittingEmi = new BigDecimal(this.advanceEmiN).multiply(findingEmi);
            	this.netDisbursedAmount = approvedLoanAmount.subtract(deriveSumTotalOfChargesDueAtDisbursement()).subtract(emittingEmi);
            
            }
            
            else{
            			
            			this.netDisbursedAmount = approvedLoanAmount.subtract(deriveSumTotalOfChargesDueAtDisbursement());
    ...
    public LoanScheduleModel regenerateScheduleModel(final ScheduleGeneratorDTO scheduleGeneratorDTO) {
        ...
        if(isAdvanceEmi()){
        		System.out.println(isAdvanceEmi());
    ...
    private LoanScheduleDTO getRecalculatedSchedule(final ScheduleGeneratorDTO generatorDTO) {
        ...
        final LoanApplicationTerms loanApplicationTerms = constructLoanApplicationTerms(generatorDTO.getApplicationCurrency(),
                ...
                generatorDTO.getCompoundingCalendarInstance(), generatorDTO.getFloatingRateDTO());
    ...
    public LoanApplicationTerms constructLoanApplicationTerms(final ApplicationCurrency applicationCurrency,
            ...
            final CalendarInstance compoundingCalendarInstance, final FloatingRateDTO floatingRateDTO) {
        final Integer loanTermFrequency = this.termFrequency;
        final PeriodFrequencyType loanTermPeriodFrequencyType = PeriodFrequencyType.fromInt(this.termPeriodFrequencyType);
        final List<DisbursementData> disbursementData = new ArrayList<>();
        for (LoanDisbursementDetails disbursementDetails : this.disbursementDetails) {
    ...
    public LoanRepaymentScheduleInstallment fetchPrepaymentDetail(final LoanScheduleGeneratorFactory loanScheduleFactory,
            ...
            final HolidayDetailDTO holidayDetailDTO, final LocalDate onDate, FloatingRateDTO floatingRateDTO) {
        LoanRepaymentScheduleInstallment installment = null;
    ...
    public LoanApplicationTerms getLoanApplicationTerms(final ApplicationCurrency applicationCurrency,
        ...
        final BigDecimal flatInterestRatePerPeriod = this.loanRepaymentScheduleDetail.getFlatInterestRatePerPeriod();
        final BigDecimal annualFlatInterestRate = this.loanRepaymentScheduleDetail.getAnnualFlatInterestRate();
